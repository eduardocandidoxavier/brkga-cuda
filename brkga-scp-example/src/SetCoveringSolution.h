...
public:
	explicit SetCoveringSolution(const std::vector< float >& chromosome,
			const bool runCover, const bool runUncover, const bool runOneOPT, float cutoff);
	~SetCoveringSolution();
	...
	void openColumn(const unsigned column, BinaryHeap< float >** refHeap);
