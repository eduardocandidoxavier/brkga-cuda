 ...
 *      Author: rtoso
 *
 *  Modified on: 2019 by Eduardo C. Xavier
 */
...
std::vector< float > SetCoveringDecoder::columnCosts;				// Cost of using each column
std::vector< unsigned > SetCoveringDecoder::rowsCoveredByColumn;	// Rows that each column covers
std::vector< std::pair< float, unsigned > > SetCoveringDecoder::sortedColumnCosts;	// Decreasing
...
SetCoveringDecoder::SetCoveringDecoder(const char* filename) {
	...
	for(unsigned i = 0; i < ncolumns; ++i) {
		...
		in >> c;
		columnCosts[i] = c;
		sortedColumnCosts.push_back(std::pair< float, unsigned >(c, i));
	}
...
float SetCoveringDecoder::decode(std::vector< float >& chromosome) const {
