...
SetCoveringSolution::SetCoveringSolution(const std::vector< float >& chromosome,
		const bool runCover, const bool runUncover, const bool runOneOPT, const float cutoff) :
		cost(0.0),
		coveredRows(0),
		openedColumns(0),
...
float SetCoveringSolution::getCost() const { return cost; }
...
bool SetCoveringSolution::greedyCover() {
	...
	BinaryHeap< float >* heap = new BinaryHeap< float >(SetCoveringDecoder::ncolumns);
	for(unsigned j = 0; j < SetCoveringDecoder::ncolumns; ++j) {
		if(rowsCoveredByCol[j] > 0) {
			heap->insert(j, - float(rowsCoveredByCol[j] / SetCoveringDecoder::columnCosts[j]));
		}
	}
...
bool SetCoveringSolution::greedyUncover() {
	bool improved = false;
	for(std::vector< std::pair< float, unsigned > >::const_iterator
...
bool SetCoveringSolution::oneOPT() {
	...
	for(unsigned i = 0; i < SetCoveringDecoder::ncolumns; ++i) {
		const float& costToLeave = SetCoveringDecoder::sortedColumnCosts[i].first;
		...
		for(unsigned j = i + 1; j < SetCoveringDecoder::ncolumns; ++j) {
			const float& costToEnter = SetCoveringDecoder::sortedColumnCosts[j].first;
...
inline void SetCoveringSolution::openColumn(const unsigned column, BinaryHeap< float >** ptrHeap) {
			...
			do {
				...
				if(ptrHeap != 0) {
					(*ptrHeap)->increaseKey(curr->column,
							- float(rowsCoveredByCol[curr->column]
